...
public class AuthEndpoints : ControllerBase
{
    ...
    public async Task<IActionResult> RegisterClient([FromBody] RegisterUserRequest request)
    {
        ...
        using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        {
            ...
            var addToRoleResult = await _userManager.AddToRoleAsync(newUser, Roles.Client);
    ...
    [HttpPost]
    [Route("registerHotelPersonnel")]
    public async Task<IActionResult> RegisterHotelPersonnel([FromBody] RegisterUserRequest request)
    {
        if (!ModelState.IsValid)
        {
            var errors = ModelState.Values.SelectMany(v => v.Errors)
        ...
        using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
        {
            ...
            var addToRoleResult = await _userManager.AddToRoleAsync(newUser, Roles.HotelPersonnel);
            if (!addToRoleResult.Succeeded)
                return StatusCode(422, "Failed to assign role to user");
            scope.Complete();
        }
